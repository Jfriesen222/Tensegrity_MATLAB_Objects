...
zeroNodes = [0,                                     0,                    0;
    r*sin(0:pi/3:5*pi/3)',       r*cos(0:pi/3:5*pi/3)',                    h*ones(6,1)];
...
for i=2:N
    ...
    spineNodes((7*i-6):(7*i),:) = quatrotate(quats(i,:),spineNodes((7*(i-1)-6):(7*(i-1)),:)  - repShift )+ repShift  + translateVector(ones(1,7),:);
